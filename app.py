...
with tabs[0]:
    ...
    if df_risk.empty:
        ...
        risk_group = df_risk.groupby("risk_label")["customer_id"].count().reset_index()
        risk_group.columns = ["risk_category", "customer_count"]
...
with tabs[5]:
    ...
    alerts_recent_sql = """
        SELECT DATE_TRUNC('hour', actual_delivery_date) AS hour,
               COUNT(*) AS total_orders,
               SUM(CASE WHEN UPPER("On Time") = 'NO' THEN 1 ELSE 0 END) AS late_orders,
               ROUND(COALESCE(SUM(CASE WHEN UPPER("On Time") = 'NO' THEN 1 ELSE 0 END)::numeric / NULLIF(COUNT(*), 0) * 100, 0), 2) AS late_rate
        FROM fact_order_line
        WHERE actual_delivery_date BETWEEN :start_date AND :end_date
        GROUP BY 1
        ORDER BY hour DESC
